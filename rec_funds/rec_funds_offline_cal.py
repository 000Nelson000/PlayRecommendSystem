...
def model_eval(train, test, users_feats_sp):
    ...
    ### popular ####
    dt_pop,model_pop = build_model(train,kind='popular',topK=100)
...
def recommender_lists(
    ...
    #### popular 
    dt_pop, model_pop = build_model(purchased_ui, kind='popular',topK=100)
    ...
    predall_ufs = model_ufs.predict(uids, topN=10)
    predall_pop = model_pop.predict(uids, topN=10)
    ...
    df_rec_total['rank'] = rec_total.index + 1
    
    t1 = time.time()
    dt_arange_df = t2 - t1 # sec
