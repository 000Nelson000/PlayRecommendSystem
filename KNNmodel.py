...
class KNNmodel:
    ...
    def __init__(self, interaction, kind='ubcf'):
        ...
        # u-i interaction data, should be user in rows, items in columns
    ...
    def _bm25_weight(self, K1=100, B=0.8):
        ...
        ret.data = ret.data * \
            (K1 + 1.0) / (K1 * length_norm[ret.row] + ret.data) * idf[ret.col]
        return ret
    ...
    def _replace_purcashed_items(self, X, remove=True):
        ...
        pur_coo = sp.coo_matrix(
            (data_pur, (row_pur, col_pur)), shape=X_coo.shape)
...
#                pred[user,:] /= np.sum(np.abs(sim[user,:])) # extremely slow
            ...
            if normalize:
                ...
                # which is zero in denominator is not allowd
                zero_idx = np.where(np.isclose(denominator, 0))
                denominator[zero_idx[0], ] = np.full(
                    (len(zero_idx[0]), 1), eps)
                pred /= denominator
...
#                pred[:,item] /= np.sum(np.abs(sim[:,item])) # extremely slow
            ...
            if normalize:
                ...
                zero_idx = np.where(np.isclose(
                    denominator, 0))  # which is zero
                denominator[zero_idx] = np.full((1, len(zero_idx[0])), eps)
                pred /= denominator

        elif self.kind == 'popular':
            num_of_sell_per_item = np.array(
                self.inter.sum(axis=0).tolist()[0])  # np-array
            ...
            for idx in range(user_features.shape[1]):
                ...
                csr_rows_set_nz_to_val(
                    inter_temp, rows_zeros, value=0)  # set to zero
    ...
    def predict(self, uids, topN=10):
        ...
        if self.kind in ('ibcf', 'ubcf', 'popular', 'ubcf_fs'):
            ...
            for _idx, _id in enumerate(uids):
                topNarray[_idx, :] = np.argsort(self.rating[_id, :].A, kind='heapsort')[
                    :, :-topN - 1:-1]
