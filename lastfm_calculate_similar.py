...
def jaccard(plays):
    ...
    rows_sum = plays.getnnz(axis=1)
    ab = plays.dot(plays.T) 
    # for rows
    aa = numpy.repeat(rows_sum, ab.getnnz(axis=1))
    # for columns
...
def main():
    ...
    methods = {'bm25': bm25, 'cosine': cosine, 'ochiai': ochiai,
               'bhattacharya': bhattacharya, 'jaccard': jaccard}
    ...
    with open("similar_artists.tsv", "w",encoding='utf8') as o:
